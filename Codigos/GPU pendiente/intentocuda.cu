...
__global__ void matrix_add(double *CudaConsultas, size_t pitch_Consultas, int N_QUERIES){
    ...
    for (col=0; col < N_QUERIES; col++)
        ((double *)((char *)CudaConsultas + row*pitch_Consultas))[col] = 1;
}
...
main(int argc, char *argv[]){
    ...
    cudaMemcpy2D(CudaDB, pitch_DB, *DB_in, sizeof(double)*N_DB, sizeof(double)*DIM, DIM, cudaMemcpyHostToDevice);
    cudaMemcpy2D(CudaConsultas, pitch_Consultas, *Consultas_in, sizeof(double)*N_QUERIES, sizeof(double)*DIM, DIM, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy2D(*Consultas_in, sizeof(double)*DIM, CudaConsultas, pitch_Consultas, sizeof(double)*N_QUERIES, DIM, cudaMemcpyDeviceToHost);
    cudaMemcpy2D(*DB_in, sizeof(double)*DIM, CudaDB, pitch_DB, sizeof(double)*N_DB, DIM, cudaMemcpyDeviceToHost);

    printf("----------------------> Prueba <-----------------------------\n");

    /*for( i = 0; i < DIM; i++ ){
        for( j = 0; j < N_DB; j++ ){
            printf("%d -\n", CudaConsultas[i*N_DB+j]);
        }
        printf("\n");
    }*/
    for( i = 0; i < DIM; i++ ){
        for( j = 0; j < N_QUERIES; j++ ){
            printf("%lf -", Consultas[i][j]);
        }
        printf("\n");
    }
